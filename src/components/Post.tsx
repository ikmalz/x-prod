...
type Engagement = {
    _count: { likes: number; rePosts: number; comment: number };
    likes: { id: number }[];
    rePosts: { id: number }[];
    saves: { id: number }[];
};
        ...
        <div className="p-4 border-y-[1px] border-borderGray">
            ...
            <div className={`flex gap-4 ${type === "status" && "flex-col"}`}>
                ...
                <div className={`${type === "status" && "hidden"} relative w-10 h-10 rounded-full overflow-hidden -z-10`}>
                    <Image path={originalPost.user.img || "general/user.png"} alt="" w={100} h={100} tr={true} />
                </div>
                {/* CONTENT */}
                <div className="flex-1 flex flex-col gap-2">
                    ...
                    <div className="w-full flex justify-between">
                        <Link href={`/${originalPost.user.username}`} className="flex gap-4">
                            ...
                            <div className={`flex items-center gap-2 flex-wrap ${type === "status" && "flex-col gap-0 !items-start"}`}>
                                <h1 className="font-bold text-md">{originalPost.user.displayName}</h1>
                    ...
                    <Link href={`/${originalPost.user.username}/status/${originalPost.id}`}>
                        <p className={`${type === "status" && "text-base"}`}>
                            {originalPost.desc}
                        </p>
                    </Link>
                    {originalPost.img && (
                        <Image
                            ...
                            className={`${originalPost.isSensitive ? "blur-xl" : ""} rounded-lg`}
                        />
                    )}
                    {type === "status" && <span className="text-textGray text-sm">10:40 PM . Jan 5, 2025</span>}
                    <PostInteractions
